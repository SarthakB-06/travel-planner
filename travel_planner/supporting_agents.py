...
place_agent = Agent(
    model="gemini-2.5-flash-lite",
    name="places_agent",
    description="Finds nearby places like hotels, cafes, attractions based on user preferences.",
    instruction="""
...
inspiration_agent = Agent(
    ...
    description="Inspires users with travel ideas, destinations and activities.",
    instruction="""
...
""",
    tools=[AgentTool(agent=news_agent) , AgentTool(agent=place_agent)],
)

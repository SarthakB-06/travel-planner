...
search_agent = Agent(
    ...
    instruction= """
        Answer the user's question directly using Google Search  grounding tool; Provide a brief but concise response. 
...
def get_nearby_locations(query:str , location:str , radius:int = 3000 , limit: int = 5)-> str:
    """
    find nearby places using only free openstreet apis (no api key needed)
...
location_search_tool = FunctionTool(func=get_nearby_locations)